...
class TargetAlert {
  ...
  TargetAlert() { _player.setReleaseMode(ReleaseMode.stop); }
  ...
  Future<void> check({required int count, required int target, required bool soundEnabled, required bool vibrationEnabled}) async {
    if (count <= 0) return;
    if (count >= target && !_playedFinal) {
      ...
      if (soundEnabled) _playSound(0.8, 2);
